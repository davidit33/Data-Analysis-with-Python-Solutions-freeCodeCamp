...
def calculate_demographic_data(print_data=True):
    ...
    higher_education_rich = round((df[filt_rich_high_educ]['salary'].count() / df[filt_high]['education'].count()) * 100, 1)
    filt_rich_low_educ = ((df['education'] != 'Bachelors') & (df['education'] != 'Masters') & (df['education']  != 'Doctorate')) & (df['salary'] == '>50K') 
    lower_education_rich = round((df[filt_rich_low_educ]['salary'].count() / df[filt_low]['education'].count()) * 100, 1)
